...
5. 固定提交的 `krillinai/KrillinAI` CLI 负责“语音转字幕 → 逐句翻译为巴西葡语 → 按片段生成配音”。转录、翻译模型和 TTS 供应商由服务器私密配置切换，每个任务还可单独指定音色。当前服务器由 KrillinAI 调用本地 `fasterwhisper/tiny` 转录、OpenAI 兼容接口翻译，并通过固定版本的官方 `edge-tts` 配音；转录和 TTS 供应商仍可改为 KrillinAI 支持的其他选项。任何一步失败都会停止制作，系统不再退回 pyvideotrans、Google 非正式翻译、MyMemory、系统语音或通用足球文案。

...
def clear_daily_keywords(db_path: Path, backup_dir: Path | None = None) -> dict[str, int | str]:
    ...
    backup_path = backup_root / f"{db_path.name}.bak-weekly-clear-{timestamp()}"
    shutil.copy2(db_path, backup_path)

    connection = sqlite3.connect(str(db_path))
    before = connection.execute(
        "SELECT COUNT(1) FROM hot_keywords WHERE source LIKE ?",
        ("daily_keywords:%",),
    ).fetchone()[0]
    connection.execute(
        "DELETE FROM hot_keywords WHERE source LIKE ?",
        ("daily_keywords:%",),
    )
    connection.commit()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Clear JaguarTV weekly tag keyword rows.")

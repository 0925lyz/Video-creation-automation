...
The default localization path delegates source transcription, pt-BR sentence translation, subtitle timing, and per-segment TTS to the pinned `krillinai/KrillinAI` CLI. KrillinAI provider selection lives in its ignored `config/config.toml`; a production request may override the provider-specific voice with `--krillinai-voice`. The installed Edge provider is invoked only through KrillinAI and uses the pinned official `edge-tts` package, not the retired factory TTS path. Translation or TTS failure stops production. Never invent generic narration, reuse untranslated text, or fall back to pyvideotrans, system voices, or metadata-derived scripts.

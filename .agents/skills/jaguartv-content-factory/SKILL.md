...
Remotion produces exactly one `通用版` per selected slice. It must include the configured bottom banner and exactly one orientation-appropriate endcard before the package can enter review. Do not request, render, expose, or publish an FB-specific variant; old FB files are historical records only.

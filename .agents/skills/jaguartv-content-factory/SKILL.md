...
CTA assets are a separate import-only inventory. Crawlers and candidate imports cannot write to it. Dashboard operators may import, preview, or delete CTA assets. Manual middle-cut editing and freeform text/image design are available only while an output is `READY_FOR_REVIEW`; both replace the existing generic output in place. Design layers are limited to the content sequence and are never rendered over the CTA.

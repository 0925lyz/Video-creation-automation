...
Remotion produces exactly one `通用版` per selected slice. It renders the source content without the retired fixed bottom banner, then randomly appends one active CTA whose orientation matches the source. CTA videos play for their full duration; CTA images display for two seconds. The review gate requires the CTA record and checks black, green, corrupted/glitch, and frozen content frames. Do not request, render, expose, or publish an FB-specific variant; old FB files are historical records only.

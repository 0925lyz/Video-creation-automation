...
本次补充时间：2026-08-23。快照已合并 `服务器优化完整提交` 中保留的部署/归档思路，并补入当前线上可用的服务器库存、发现素材、文案设计、公开上传、归档、待审核流程，以及“导入视频”来源标签和“下载后放入”目标区域功能代码。
...
- `server-snapshot/src/jaguartv_factory/source_imports.py`: 导入视频来源记录、URL 安全校验、媒体校验、重复检测、目标区域和审计记录。
...
- 内容库存新增“导入视频”来源筛选，并和待制作、待审核、审核通过、导入失败等实际状态组合过滤。
- 发现素材/导入素材弹窗新增“下载后放入”：默认进入待制作；选择审核通过时走权限校验、风险提示、二次确认和后端媒体校验。
- 待制作导入视频进入统一智能切片、通用版、FB版、媒体质量校验、待审核流程；审核通过导入视频记录为外部导入成片，不伪造成自动二创产物。

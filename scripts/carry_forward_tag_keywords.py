...
def carry_forward(db_path: Path, date_value: str, backup_dir: Path | None = None) -> dict[str, int | str]:
    ...
    with sqlite3.connect(str(db_path)) as connection:
        existing = connection.execute(
            "SELECT COUNT(1) FROM hot_keywords WHERE date=? AND source LIKE 'daily_keywords:%'",
            (date_value,),
        ).fetchone()[0]
        if existing:
            return {"copied": 0, "target_existing": existing, "backup": str(backup_path)}
        source_row = connection.execute(
            "SELECT date FROM hot_keywords WHERE source LIKE 'daily_keywords:%' AND date < ? "
            "GROUP BY date ORDER BY date DESC LIMIT 1",
            (date_value,),
        ).fetchone()
        if not source_row:
            return {"copied": 0, "target_existing": 0, "backup": str(backup_path)}
        source_date = str(source_row[0])
        source_rows = connection.execute(
            "SELECT keyword,source FROM hot_keywords WHERE date=? AND source LIKE 'daily_keywords:%'",
            (source_date,),
        ).fetchall()
        created_at = datetime.now(ZoneInfo("UTC")).isoformat()
        connection.executemany(
            "INSERT OR IGNORE INTO hot_keywords(keyword,date,source,created_at) VALUES(?,?,?,?)",
            [(keyword, date_value, source, created_at) for keyword, source in source_rows],
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Carry latest tag keywords forward to today.")

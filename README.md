...
- 智能切片：音频峰值 35% + 运动 25% + 镜头变化 20% + 关键词 15% + 回放 5%；每段硬性不超过 30 秒。
- Reaction：画中画、上下分屏、左右分屏；Reaction 不覆盖品牌尾卡。
- 本地化策略：KrillinAI 可切换转录、翻译模型和 TTS 供应商，并支持按任务指定音色；Tesseract 只检测原字幕位置，不模糊画面；中文人声和中文字幕同时命中时，Demucs 移除人声并保留伴奏，再混入 pt-BR 配音。失败即停止，不生成通用兜底文案。
...
Remotion 为每个审核包只生成一个 `通用版`。正文固定显示 `assets/brand/jaguartv_download_banner.jpg`：横屏素材紧贴视频画面上沿，竖屏素材固定在画面顶部；它不会出现在 CTA 结尾。巴西葡语字幕根据检测到的原字幕区域自动选择上方或下方空位。结尾从独立 CTA 库随机选择一个与源视频方向一致的素材，CTA 视频按原时长完整播放，CTA 图片显示 2 秒。

...
Remotion 品牌模式会为每个审核包生成一个 `通用版`：视频画面下方保留宣传条，结尾追加一次与画面方向匹配的宣传尾图。

默认预览文件仍是 `review/<candidate>/video.mp4`。内容库存页只把 `通用版` 作为可预览、下载和发布的自动制作成片；历史 FB 文件不会被删除，但不再出现在可用成片列表中。

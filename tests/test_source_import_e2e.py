...
def test_finished_upload_exposes_original_source_and_submits_it():
    ...
    assert "const canApprove = mode === \"upload\" || !!state.importCapabilities.can_direct_approve" in javascript
    assert "const canDirectApprove = mode === \"upload\" || !!state.importCapabilities.can_direct_approve" in javascript
